...
exports.handler = async function(event) {
  ...
  let handle = process.env.SECRET_HANDLE_f51438d0;
  if (!handle) {
    throw new Error("Missing environment variable: SECRET_HANDLE_f51438d0");
  }
  ...
  let handle = process.env.SECRET_HANDLE_095c4e47;
  if (!handle) {
    throw new Error("Missing environment variable: SECRET_HANDLE_095c4e47");
  }
  const simulatorUrl = process.env.WING_SIMULATOR_URL;
  if (!simulatorUrl) {
    throw new Error("Missing environment variable: WING_SIMULATOR_URL");
